      ...
      <div className="content-wrapper">
        ...
        <div className="glass-card">
          <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem" }}>
            <button className={`btn ${activePanel === "home" ? "btn-primary" : "btn-outline"}`} onClick={() => setActivePanel("home")}>
              📋 Meus Artigos
            </button>
            <button className={`btn ${activePanel === "articles" ? "btn-primary" : "btn-outline"}`} onClick={() => setActivePanel("articles")}>
              ✍️ Submeter Artigo
            </button>
          </div>
          ...
          {activePanel === "home" && (
            <div className="table-wrapper">
              <table>
                <thead><tr><th>Título</th><th>Área</th><th>Data</th></tr></thead>
                <tbody>
                  {artigos.length === 0 ? (
                    <tr><td colSpan={3} style={{ textAlign: "center" }}>Nenhum artigo submetido</td></tr>
                  ) : (
                    artigos.map(a => (
                      <tr key={a.id}>
                        <td>{a.titulo}</td>
          ...
          {activePanel === "articles" && (
            <form onSubmit={handleSubmitArtigo}>
              <div className="form-grid">
                <div className="form-group full-width"><label className="form-label">Título *</label><input className="form-input" value={artForm.titulo} onChange={e => setArtForm({ ...artForm, titulo: e.target.value })} required /></div>
                <div className="form-group full-width"><label className="form-label">Resumo *</label><textarea className="form-textarea" rows={5} value={artForm.resumo} onChange={e => setArtForm({ ...artForm, resumo: e.target.value })} required /></div>
                <div className="form-group"><label className="form-label">Palavras-chave</label><input className="form-input" value={artForm.palavrasChave} onChange={e => setArtForm({ ...artForm, palavrasChave: e.target.value })} /></div>
                <div className="form-group"><label className="form-label">Área *</label>
                  <select className="form-select" value={artForm.area} onChange={e => setArtForm({ ...artForm, area: e.target.value })} required>
                ...
                <div className="form-group full-width"><label className="form-label">Arquivo (PDF/DOCX)</label><input type="file" accept=".pdf,.doc,.docx" onChange={e => setSelectedFile(e.target.files?.[0] || null)} /></div>
              </div>
